...
def draw_HST_profiles(
    ...
    if mag_cut is not None:
        mag_cut_list = np.where(
            catalog.param_cat[catalog.orig_index]["mag_auto"] > mag_cut
        )[0]
    ...
    for i, ind in enumerate(indices):
        ...
        gal_kimage_ = gal_.drawKImage(
            nx=Npx, ny=Npx, scale=2 * np.pi / pixel_scale_hst / Npx
        ).array
...
def draw_AE_HST_profiles(
    ...
    obs_list, psf_list = [], []
    for ind in indices:
        gal_real = catalog.makeGalaxy(
            int(ind), gal_type="real", noise_pad_size=Npx * pixel_scale_hst
        )
        ...
        obs_im = (
            gs.Convolve(gal_real, psf)
            .drawImage(nx=Npx, ny=Npx, scale=pixel_scale_hst, method="no_pixel")
            .array.astype("float32")
        )
    ...
    im_gal = []
    for i in range(Ngal):
        gal_ = galsim.InterpolatedImage(
            galsim.Image(g_dec_np[i, 0], scale=pixel_scale_vae)
        )
        gal_ = gal_.shear(g1=g1, g2=g2)
...
def draw_NN_profile(
    ...
    y = jitted_decode(z[None, :, :], key=key if use_dropout else None).astype(
        jnp.float32
    )
...
def sample_galaxy_params(
    ...
    if TRECS_fit_dir is not None:
        ...
        flux_rate = (
            1 / flux_fit["scale"]
        )  # Exponential distribution in scipy is parameterized by scale, while in numpyro it's parameterized by rate (1/scale)
        flux_shift = flux_fit["loc"]
        flux = (
            numpyro.sample("flux", dist.Exponential(flux_rate), sample_shape=(Ngal,))
            + flux_shift
        )
        ...
        raise ValueError(
            "Either TRECS_fit_dir or deepshape_dataset_dir must be provided."
        )
...
def gen_gal_dataset(
    ...
    if profile_type == "VAE":
        ...
        if cosmos_dataset_dir is None:
            raise ValueError(
                "cosmos_dataset_dir must be provided when profile_type='VAE'"
            )
    ...
    if profile_type == "real" or profile_type == "cosmos":
        ...
        if profile_type == "exp" or profile_type == "sersic":
            draw = partial(
                draw_sersic_profile, uv_pos=uv_pos, Npx=Npx, pixel_scale=pixel_scale
            )
        elif profile_type == "spergel":
